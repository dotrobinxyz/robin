...
/// @title RobinWrapper
/// @notice ERC-1155 wrapping, fuses, and subdomain permissions for .robin
///         names — what makes subdomains real, tradeable assets.
/// @dev Copy of upstream NameWrapper (ens-contracts v1.7.0) with the following
///      deliberate diffs — everything else, including all fuse logic, is
///      verbatim (public function names such as wrapETH2LD are intentionally
///      retained for upstream interface/tooling compatibility; "ETH2LD" reads
///      as "second-level name under the base TLD"):
///      1. TLD constants: ROBIN_NODE = namehash('robin'), ROBIN_LABELHASH =
///         keccak256('robin'), and the DNS-encoded suffix "\x05robin\x00"
///         (two sites: constructor and _wrapETH2LD).
///      2. GRACE_PERIOD is immutable, read from the Robin base registrar at
///         deploy (mainnet: 90 days, the upstream value) so testnet lifecycle
///         rehearsals can run with shortened timers.
///      3. Collection name: "Robin Wrapped Names".
///      4. Doc comments reference .robin instead of .eth.
contract RobinWrapper is
    Ownable,
    ERC1155Fuse,
    INameWrapper,
    Controllable,
    ...
    ReverseClaimer
{
    ...
    string public constant name = "Robin Wrapped Names";

    // Robin diff: immutable, read from the registrar at deploy so the two
    // can never disagree. Mainnet value: 90 days (the upstream constant).
    // Private (no getter) to stay under the EIP-170 size limit; read the
    // registrar's public GRACE_PERIOD instead.
    uint64 private immutable GRACE_PERIOD;
    bytes32 private constant ROBIN_NODE =
        0x1a9af74db203c4017d8445942e9b64ce93d8bc2ae2eed5b8dcbbb0090690d2b3;
    bytes32 private constant ROBIN_LABELHASH =
    ...
    constructor(
        ...
        GRACE_PERIOD = uint64(
            RobinBaseRegistrar(address(_registrar)).GRACE_PERIOD()
        );
        metadataService = _metadataService;

        /* Burn PARENT_CANNOT_CONTROL and CANNOT_UNWRAP fuses for ROOT_NODE and ROBIN_NODE and set expiry to max */
    ...
    /// @param id Label as a string of the .robin domain to wrap
    ...
    /// @notice Wraps a .robin domain, creating a new token and sending the original ERC721 token to this contract
    /// @dev Can be called by the owner of the name on the .robin registrar or an authorised caller on the registrar
    /// @param label Label as a string of the .robin domain to wrap
    ...
    function wrapETH2LD(
        ...
        if (
            ...
            revert Unauthorised(
                _makeNode(ROBIN_NODE, bytes32(tokenId)),
                msg.sender
            );
        }
    ...
    /// @dev Registers a new .robin second-level domain and wraps it.
    ...
    /// @param ownerControlledFuses Initial owner-controlled fuses to set
    /// @return registrarExpiry The expiry date of the new name on the .robin registrar, in seconds since the Unix epoch.
    ...
    /// @notice Renews a .robin second-level domain.
    /// @dev Only callable by authorised controllers.
    /// @param tokenId The hash of the label to register (eg, `keccak256('foo')`, for 'foo.robin').
    /// @param duration The number of seconds to renew the name for.
    /// @return expires The expiry date of the name on the .robin registrar, in seconds since the Unix epoch.
    function renew(
        uint256 tokenId,
        uint256 duration
    ) external onlyController returns (uint256 expires) {
        bytes32 node = _makeNode(ROBIN_NODE, bytes32(tokenId));
    ...
    /// @notice Wraps a non .robin domain, of any kind. Could be a DNSSEC name vitalik.xyz or a subdomain
    ...
    /// @notice Unwraps a .robin domain. e.g. vitalik.robin
    /// @dev Can be called by the owner in the wrapper or an authorised caller in the wrapper
    /// @param labelhash Labelhash of the .robin domain
    /// @param registrant Sets the owner in the .robin registrar to this address
    /// @param controller Sets the owner in the registry to this address
    function unwrapETH2LD(
        bytes32 labelhash,
        address registrant,
        address controller
    ) public onlyTokenOwner(_makeNode(ROBIN_NODE, labelhash)) {
        if (registrant == address(this)) {
            revert IncorrectTargetOwner(registrant);
        }
        _unwrap(_makeNode(ROBIN_NODE, labelhash), controller);
    ...
    /// @notice Unwraps a non .robin domain, of any kind. Could be a DNSSEC name vitalik.xyz or a subdomain
    /// @dev Can be called by the owner in the wrapper or an authorised caller in the wrapper
    ...
    /// @notice Upgrades a domain of any kind. Could be a .robin name vitalik.robin, a DNSSEC name vitalik.xyz, or a subdomain
    ...
    function _beforeTransfer(
        ...
        // For this check, treat .robin 2LDs as expiring at the start of the grace period.
        if (fuses & IS_DOT_ETH == IS_DOT_ETH) {
            expiry -= GRACE_PERIOD;
        }
    ...
    function _normaliseExpiry(
        ...
        // .robin names will check registrar, non .robin check parent
        if (expiry > maxExpiry) {
            expiry = maxExpiry;
        }
    ...
    function _wrapETH2LD(
        ...
        bytes32 labelhash = keccak256(bytes(label));
        bytes32 node = _makeNode(ROBIN_NODE, labelhash);
        // hardcode dns-encoded robin string for gas savings
        bytes memory name = _addLabel(label, "\x05robin\x00");
        names[node] = name;
